	...
	<head>
		...
		{{each dataList as row}}
			<tr>
				...
				<td>{{row.memo}}</td>
				<td>
					<a class="btn btn-success" href="javascript:Public.goAdd('${webRoot}/sysResource/goAdd.do?pid={{row.id}}')">
						<i class="fa fa-search-plus "></i>                                            
					</a>
					<a class="btn btn-info" href="javascript:Public.goEdit('${webRoot}/sysResource/goEdit.do?id={{row.id}}')">
						<i class="fa fa-edit "></i>                                            
					</a>
					<a class="btn btn-danger" href="javascript:Public.ajaxDel('${webRoot}/sysResource/delete.do?id={{row.id}}')">
		...
		{{each dataList as row}}
			<tr class="trChild">
				<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="fa  fa-terminal"></i>&nbsp;{{row.name}}</td>
				<td>{{row.url}}</td>
				<td>{{row.memo}}</td>
				<td>
					<a class="btn btn-info" href="javascript:Public.goEdit('${webRoot}/sysResource/goEdit.do?id={{row.id}}')">
			...
			<div class="panel panel-default">
				<div id="panel-heading" class="panel-heading">
					...
					<div class="panel-actions">
						<a href="javascript:Public.goAdd('${webRoot}/sysResource/goAdd.do')" class="btn-setting" ><i class="fa fa-plus"></i></a>
				...
				<div class="panel-body">
					<div class="searchbar">
			          <div class="searchForm">
			          	...
			          	<div class="form-group">
		                    <label class="col-sm-4 control-label" for="name">资源名称:</label>
		                    <div class="col-sm-8">
					...
					<table class="table table-striped table-bordered bootstrap-datatable datatable ltable" url="${webRoot}/sysResource/datagrid.do" sort="name" order="desc">
					  <thead>
						  <tr>
							  <th sort="name">资源名称</th>
...
<script type="text/javascript">
	function child(that,id){
		...
		jQuery.ajax({
			url : "${webRoot}/sysResource/datagridChild.do",
				...
				if(tr.next().attr("class")=="trChild"){
				 ...
				 if(ret.code=='0000'){
					 ...
					 if(result.totalCount>0){
						 ...
						 $(that).parent().after("<tr class='trChild'><td height='40px;' colspan='100' align='center'><span><i class=\"fa  fa-terminal\"></i>&nbsp;抱歉！无子栏目</span></td></tr>");
					 }
				 }else{
					 alert("查询错误");
				 }
