package cn.posolft.framework.utils;
...
public class BufferedRandomAccessFile extends RandomAccessFile {
	...
	public boolean write(byte bw, long pos) throws IOException {
		...
		if ((pos >= bufstartpos) && (pos <= bufendpos)) {
			...
			if ((pos >= 0) && (pos <= fileendpos) && (fileendpos != 0)) {
				buf[(int) (pos - bufstartpos)] = bw;
			} else if (((pos == 0) && (fileendpos == 0)) || (pos == fileendpos + 1)) { // write
																						// pos
																						// is
																						// append
																						// pos
	...
	public void seek(long pos) throws IOException {
		...
		if ((pos < bufstartpos) || (pos > bufendpos)) { // seek pos not in buf
			...
			} else if (((pos == 0) && (fileendpos == 0)) || (pos == fileendpos + 1)) { // seek
																						// pos
																						// is
																						// append
																						// pos
	...
	public static void main(String[] args) throws IOException {
		...
		brafReadFile = new BufferedRandomAccessFile("G:/迅雷下载/Office_Professional_Plus_2013.1546552308.zip");
		readfilelen = brafReadFile.initfilelen;
		brafWriteFile = new BufferedRandomAccessFile("D:/Office_Professional_Plus_2013.1546552308.zip", "rw", 10);
		...
		System.out.println("BufferedRandomAccessFile Copy & Write File: " + brafReadFile.filename + "    FileSize: " + java.lang.Integer.toString((int) readfilelen >> 1024)
				+ " (KB)    " + "Spend: " + (double) (System.currentTimeMillis() - start) / 1000 + "(s)");

		// java.io.FileInputStream fdin = new
		// java.io.FileInputStream("C:/1.7z");
		// java.io.BufferedInputStream bis = new
		// java.io.BufferedInputStream(fdin, 1024);
		// java.io.DataInputStream dis = new java.io.DataInputStream(bis);
		//
		// java.io.FileOutputStream fdout = new
		// java.io.FileOutputStream("D:/STKAITI_002.7z");
		// java.io.BufferedOutputStream bos = new
		// java.io.BufferedOutputStream(fdout, 1024*100);
		...
		// java.lang.Integer.toString((int) readfilelen >> 1024) + " (KB)    "
		// + "Spend: " + (double) (System.currentTimeMillis() - start) / 1000 +
		// "(s)");
	}
}
